...
impl RenderDims {
    fn new(
        ...
        let mut scalar_from_min =
            (native_dims.0 as f64 / min_dims.0).min(native_dims.1 as f64 / min_dims.1);
        let mut scalar_from_max =
            (native_dims.0 as f64 / max_dims.0).max(native_dims.1 as f64 / max_dims.1);
        ...
        let used_native_dims = (
            native_dims
                .0
                .min((max_dims.0 * pixel_scalar).floor() as u32),
            native_dims
                .1
                .min((max_dims.1 * pixel_scalar).floor() as u32),
        );
        let native_pad = (
            native_dims.0 - used_native_dims.0,
            native_dims.1 - used_native_dims.1,
        );
        let native_pre_pad = (native_pad.0 / 2, native_pad.1 / 2);
        let dims = (
            used_native_dims.0 as f64 / pixel_scalar,
            used_native_dims.1 as f64 / pixel_scalar,
        );
        RenderDims {
            min_dims,
            max_dims,
