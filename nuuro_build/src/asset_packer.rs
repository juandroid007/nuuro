...
impl AssetPacker {
    ...
    /// the current directory used when the Nuuro application is run.
    ...
    pub fn cargo_rerun_if_changed(&mut self) {
        assert!(
            self.sprites.is_none() && self.music.is_none() && self.sounds.is_none(),
    ...
    pub fn mp3_fallback(&mut self) {
        assert!(
            self.music.is_none() && self.sounds.is_none(),
            "cannot set mp3 fallback after audio asset packing has already started"
        );
        self.mp3_fallback = true;
    }

    /// Packs sprite images into an atlas, to be rendered by Nuuro renderer in "sprite" mode.
    ///
    ...
    ///
    /// Note: in the current implementation of `nuuro` there is only one atlas for sprites,
    ...
    pub fn sprites(&mut self, in_dir: &Path) -> &[String] {
        assert!(
            ...
            "self.sprites(...) was already invoked"
        );
    ...
    pub fn music(&mut self, in_dir: &Path) -> &[String] {
        ...
        self.music = Some(enumerate_audio(
            in_dir,
            &self.assets_dir,
            "music",
            self.mp3_fallback,
    ...
    pub fn sounds(&mut self, in_dir: &Path) -> &[String] {
        assert!(
            self.sounds.is_none(),
            "self.sounds(...) was already invoked"
        );
        self.sounds = Some(enumerate_audio(
            in_dir,
            &self.assets_dir,
            "sound",
            self.mp3_fallback,
    ...
    /// Creates a "nuuro.js" and "index.html" file in the assets directory for use with the
    /// WASM target architecture.
    ///
    /// Build a nuuro app for WASM using the command
    ...
    /// to "my_assets_dir/nuuro_app.wasm".
    ...
    /// which implements `nuuro::asset_id::AppAssetId`.
    /// Constructing a `nuuro::App` instance with this as the Asset ID type
    ...
    pub fn gen_asset_id_code(self, out: &Path) {
        self.gen_asset_id_code_checked(out)
            .expect("error generating AssetId code")
    }

    fn gen_asset_id_code_checked(self, out: &Path) -> io::Result<()> {
        let sprites_enum = gen_asset_enum(
            "SpriteId",
            &self.sprites.expect("self.sprites(...) was not invoked"),
        );
        let music_enum = gen_asset_enum("MusicId", &self.music.unwrap_or_else(|| vec![]));
        let sounds_enum = gen_asset_enum("SoundId", &self.sounds.unwrap_or_else(|| vec![]));

        let code = format!(
            include_str!("asset_id.template.rs"),
            sprites_enum, music_enum, sounds_enum
        );
...
fn gen_asset_enum(name: &str, ids: &[String]) -> String {
    ...
    if ids.is_empty() {
        ...
        format!(
            include_str!("asset_id_u8.template.rs"),
            name,
            ids_str,
            ids.len()
        )
        ...
        format!(
            include_str!("asset_id_u16.template.rs"),
            name,
            ids_str,
            ids.len()
        )
