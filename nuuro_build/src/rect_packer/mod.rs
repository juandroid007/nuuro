...
fn log_ceil_pow_2(value: u32) -> u32 {
    (0..32)
        .find(|&log| 1 << log >= value)
        .expect("value is too large")
}
...
fn overall_dims(rects: &[Rect]) -> (u32, u32) {
    (
        rects
            .iter()
            .map(|r| ceil_pow_2(r.pos.0 + r.dims.0))
            .max()
            .unwrap_or(0),
        rects
            .iter()
            .map(|r| ceil_pow_2(r.pos.1 + r.dims.1))
            .max()
            .unwrap_or(0),
    )
}
