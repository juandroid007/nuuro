...
use crate::asset_id::{AssetId, MusicId, SoundId, SpriteId};
...
fn disc_pos(pillar_index: usize, height_index: usize) -> (f64, f64) {
    (
        -27. + pillar_index as f64 * 27.,
        2.5 + height_index as f64 * 5.,
    )
}
...
fn pillars_initial_position() -> Vec<Vec<u8>> {
    vec![vec![4, 3, 2, 1, 0], vec![], vec![]]
}

struct TowerGame {
    pillars: Vec<Vec<u8>>,
    held: Option<HeldDisc>,
}

impl TowerGame {
    pub fn new() -> Self {
        Self {
            pillars: pillars_initial_position(),
            held: None,
        }
    }
}
...
impl App<AssetId> for TowerGame {
    ...
    fn key_down(&mut self, key: KeyCode, ctx: &mut AppContext<AssetId>) {
        ...
        if key == KeyCode::R {
            let is_initial = self.held.is_none();
            if is_initial || self.pillars == pillars_initial_position() {
                self.pillars = pillars_initial_position();
        ...
        if let Some(index) = index {
            ...
            } else if let Some(value) = pillar.pop() {
    ...
    fn render(&mut self, renderer: &mut Renderer<AssetId>, ctx: &AppContext<AssetId>) {
        ...
        for (pillar_index, pillar) in self.pillars.iter().enumerate() {
            for (height_index, _) in pillar.iter().enumerate() {
                let pos = disc_pos(pillar_index, height_index);
                renderer.draw(
                    &base.pre_translate(pos.0, pos.1),
                    disc_sprite(pillar[height_index]),
                )
            }
        }
        ...
        if let Some(held) = self.held.as_ref() {
            renderer.draw(
                &base.pre_translate(held.pos.0, held.pos.1),
                disc_sprite(held.value),
            );
        }
    }
}
